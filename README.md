...
## Viktigt: ni kör Cloudflare Workers (inte klassiska "Pages")

Cloudflare har slagit ihop sina produkter, och er sajt (`launchuf.kontakt-launchuf.workers.dev`) körs som en **Worker med statiska filer**, kopplad direkt till ert GitHub-repo. Det fungerar precis lika bra, men tre filer i den här zippen är till för just det:

- **`_worker.js`** — den "lilla servern". Cloudflare kör den här filen för varenda request till sajten. Den kollar om URL:en är `/create-checkout-session` eller `/stripe-webhook` (då kör den vår betalningskod), annars skickar den vidare till era vanliga filer (index.html, styles.css, osv.)
- **`server/create-checkout-session.js`** och **`server/stripe-webhook.js`** — själva betalningslogiken, importeras av `_worker.js`
- **`wrangler.toml`** — talar om för Cloudflare var de statiska filerna ligger så `_worker.js` kan servera dem

Ni behöver bara pusha allt till GitHub som vanligt — Cloudflare bygger om automatiskt, precis som ni redan sett i era Deployments.
...
2. Lägg till en användare för dig och en för William (e-post + valfritt lösenord) — **bocka i "Auto Confirm User"**, annars går det inte att logga in förrän e-posten bekräftats
...
1. Gå till er Worker i Cloudflare-dashboarden (samma vy som i din skärmdump: **Workers & Pages → launchuf**)
2. Klicka på fliken **Settings** → leta upp **"Variables and Secrets"** (kan också heta bara "Variables" beroende på var i Cloudflares UI ni är)
3. Lägg till (välj typen **"Secret"**, inte "Text/Plaintext"):

| Namn | Värde |
|---|---|
| `STRIPE_SECRET_KEY` | er `sk_test_...` eller `sk_live_...` från Stripe |
| `STRIPE_WEBHOOK_SECRET` | se steg 6 nedan |
| `SUPABASE_URL` | er Supabase-projekt-URL (samma som i config.js) |
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase → Project Settings → API → den hemliga "secret key" (`sb_secret_...`) |
| `SITE_URL` | `https://launchuf.kontakt-launchuf.workers.dev` (eller er egen domän om ni kopplat en) |

4. Spara. Om Cloudflare inte automatiskt startar om Workern, gör en ny commit (även en liten) för att trigga en ny deployment så den plockar upp de nya värdena.
...
2. Endpoint-URL: `https://launchuf.kontakt-launchuf.workers.dev/stripe-webhook`
...
| `_worker.js` | Routrar requests: /create-checkout-session och /stripe-webhook går till er kod, allt annat blir vanliga statiska filer |
| `server/create-checkout-session.js` | Skapar Stripe-betalningen |
| `server/stripe-webhook.js` | Tar emot bekräftelse från Stripe och markerar ordern betald |
| `wrangler.toml` | Talar om för Cloudflare hur de statiska filerna ska bindas till `_worker.js` |
