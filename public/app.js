  ...
  .forEach(btn => {
    ...
    btn.addEventListener(
      ...
      e => {
        ...
        btn.style.transform =
          `translate(${(e.clientX - r.left - r.width / 2) * .12}px, ${(e.clientY - r.top - r.height / 2) * .3}px)`;
...
if (window.gsap) {
  ...
  gsap.from(
    '.hero-eyebrow, .hero-lead, .hero-actions',
    {
      y: 16,
      opacity: 0,
      duration: .9,
...
(() => {
  ...
  async function submitSupabase(data) {
    ...
    if (file) {
      ...
      const path =
        `${new Date().getFullYear()}/${crypto.randomUUID()}-${clean}`;
  ...
  const stripeConfigured =
    Boolean(
      ...
      !cfg.stripePublishableKey.includes('YOUR_')
      ...
      try {
        ...
        if (payNow) {
          ...
          if (!stripeConfigured) {
            ...
            throw new Error(
              'Betalning är tillfälligt otillgänglig. Välj Betala senare eller kontakta oss.'
            );
        ...
        alert(
          `Beställningen kunde inte skickas. ${err.message || 'Kontrollera inställningarna.'}`
        );
